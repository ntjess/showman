...
- Provide your readme in typst format with code block examples, and let `showman` do the rest! In one command, it will the readme to markdown and render code block outputs as included images.
  - Bonues points -- let `showman` know your repository path and it will ensure images will properly appear in your readme even after your package has been distributed through typst's registry.
...
After enough people test this package, I will submit it as an official typst package and it will be directly available as a `@preview` citizen.

== Usage
=== Converting your readme to markdown
Create a typst file with #raw("```example") code blocks that show the output you want to include in your readme. For instance:
...
4. Without any other arguments, you've just installed your package in your system's local typst packages folder. Now you can import it with `typst #import "@local/mypackage:<version>"`.
